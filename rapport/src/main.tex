\documentclass[]{report}

\input{../framework/sp}
\splanguage{french}
\input{extra}

\title{\tbf{Projet personnel en Informatique\\ Lyon Ray Tracer}}
\author{%
Maxime \tsc{Gaudin}\\
~\\
\small \href{mailto:maxime.gaudin@polymtl.ca}{maxime.gaudin@polymtl.ca}\\
\small Institue National des Sciences Appliquées de Lyon (INSA)\\
\small École Polytechnique de Montréal
}
\customdate

\begin{document}
\maketitle

\input{abstract} % CHECKED

\input{introduction} % CHECKED

\input{objectives} % CHECKED

\input{state_of_the_art} % CHECKED

\input{architecture} % CHECKED

\input{results} % CHECKED

\input{manual} % CHECKED

\input{conclusion} % CHECKED

\glsaddall
\printglossaries

\bibliography{references}

\tableofcontents

\listoffigures 

\newpage
\vspace*{\fill}
\quote[Donald Knuth \smiley]{S'il est plus près du programme de \tsc{Whitted} que de
\tsl{Renderman}\footnote{Pixar}, \tsl{LRT} reste très prometteur !}
\vspace*{\fill}
\end{document}
